...
IF ~~ THEN BEGIN 0
SAY ~Mein Stimme ist nix ohne Aufgabe. Mein Stimme ist wichtig, wenn alle haben Futter, aber ohne Aufgabe da ist kein Futter.~
IF ~~ THEN EXTERN ~NQ#SHUK~ 19
END

IF ~~ THEN BEGIN 1
SAY ~Hähnchen sind in Fässern, aber Trottel bringen falsche Fässer. Richtige im Lager sein, ich schicke Trottel wieder, aber diesmal nicht kommen zurück. Holst du mir Viecher aus sechs Fässer!~
IF ~~ THEN DO ~SetGlobal("NQ#IZVO","GLOBAL",1) AddJournalEntry(@70032,QUEST)~ EXIT
END

IF ~~ THEN BEGIN 2
SAY ~Nein, nein, nix Problem, Chef! Ich hab Futter, Du hast mein Stimme!~
...
SAY ~Ah, Sklave von Shuk! Wo ist er?~
IF ~~ THEN EXIT
END
...
SAY ~Shuk! Du gekommen. Gut. Aufgabe kann nicht lange warten.~
IF ~~ THEN EXTERN ~NQ#SHUK~ 18
END

IF ~Global("NQ#IZVO","GLOBAL",9)~ THEN BEGIN NOSPIDER
SAY ~Bringt Hähnchen in Höhle! Leckerchen werden sie riechen und kommen!~
IF ~~ THEN EXIT
END
...
SAY ~Ah, da sind mein Hähnchen! Gut. Bringt sie jetzt in die Höhle! Ihr Geruch perfekt, damit Leckerbissen aus ihren Verstecken kommen!~
...
SAY ~Machst Du Fässer kaputt und vergisst Du dann auch nicht einsammeln mein Hähnchen!~
IF ~~ THEN EXIT
END
...
SAY ~Spinnen tot? Bringst du mir dicke Spinne!~
...
SAY ~Ja! Fette, fette Spinne! Du mir geben für fetten, fetten Eintopf!~
IF ~~ THEN EXTERN ~NQ#SHUK~ 20
END
