...
IF ~~ THEN BEGIN 0
SAY ~Hey, das bringt Dich nicht weiter, häh? Kannst mich wahrscheinlich umhauen, aber das beeindruckt hier keinen. Bin ein Schütze und kein Kämpfer, weißte?~
IF ~~ THEN GOTO 1
END

IF ~~ THEN BEGIN 1
SAY ~Tu mir nur 'nen kleinen Gefallen, ja? Dann sieht hier jeder was für'n krasser Kerl Du bist und ich werd' auch nur das Beste über Dich erzähl'n, klar?~
IF ~~ THEN EXTERN ~NQ#SHUK~ 10
END

IF ~~ THEN BEGIN 2
SAY ~Mulm ist einer von den Kämpfern, häh? Wollte, dass wir Schützen ihm unser ganzes Zeug geben. Ich hab ihn mit 'nem Pfeil erwischt, aber er hat's überlebt. Zäher Bastard! Jetzt sammelt er sein Pack um uns fertig zu machen, kapiert?~
IF ~~ THEN GOTO 3
END

IF ~~ THEN BEGIN 3
SAY ~Wenn Du ihn fertig machst, wird keiner was sagen. Prüfung der Götter und all das. Er ist mit seinen Jungs irgendwo nördlich von hier unterwegs. Halt mir den Rücken frei, dann pass' ich in Zukunft auf, dass keiner auf Deinen zielt, alles klar?~
IF ~~ THEN EXTERN ~NQ#SHUK~ 11
END

IF ~~ THEN BEGIN 4
SAY ~Oh, ähm, ja klar... Chef.~
...
IF ~~ THEN BEGIN 5
SAY ~Raki hat Probleme mit den Bäumen, klar? Meine Jungs stellen Dir ein paar Sprengfässer an den Waldrand. Kriegst'e ein Problem, machst'e einfach was Feuer dran. Bumm, hahaha!~
...
IF ~~ THEN BEGIN 6
SAY ~Izvo kümmert sich ums Fressen, aber das Fressen will lieber ihn fressen, kapiert? Meine Jungs stellen Dir ein paar Sprengfässer in die Höhle. Kriegst'e ein Problem, machst'e einfach was Feuer dran. Bumm, hahaha!~
IF ~~ THEN DO ~TakePartyGold(50) SetGlobal("NEEDUHELP","LOCALS",3) SetGlobal("NQ#PAEK","GLOBAL",6)~ GOTO 7 // Izvo
END

IF ~~ THEN BEGIN 7
SAY ~Versucht nur keine and'ren Goblins zu brutzeln, ja? Das gibt Ärger... ich hab's schon erlebt.~
IF ~~ THEN EXIT
END

IF ~~ THEN BEGIN 8
SAY ~Kein Problem, Chef!~
...
SAY ~Ihr seid mit dem Shuk unterwegs, hä? Wo ist er?~
IF ~~ THEN EXIT
END
...
SAY ~Du bist Shuk, hä? Hab von Dir gehört. Krasser Krieger sagt der Schamane...~
...
SAY ~Lasst mich in Ruhe, <RACE>. Wo ist Shuk?~
IF ~~ THEN EXIT
END
...
SAY ~Hä? Haust ihm den Kopf ab, oder was?~
IF ~~ THEN EXIT
END
...
SAY ~Hab' gehört die Sau ist tot. Sehr gut! Meine Stimme ist Dir sicher!~
...
IF ~~ THEN BEGIN HELP1
SAY ~Du wist bald sehen wie nützlich ich bin, oder? Paek ist immer für Dich da,Chef!~
...
IF ~~ THEN BEGIN HELP2
SAY ~Du hast noch nich' mit Raki und Izvo gequatscht, was? Ich weiß genau welche Probleme die haben und jeder krasse Boss braucht ein krasse rechte Hand, oder? Ich werd' fünfzig Goldstücke brauchen, dann kann ich Dir eine große Hilfe sein!~
IF ~~ THEN DO ~SetGlobal("NEEDUHELP","LOCALS",1)~ EXTERN ~NQ#SHUK~ 24
END

IF ~~ THEN BEGIN HELP3
SAY ~Du hast noch nich' mit Izvo gequatscht, oder? Ich weiß genau welche Probleme der hat und jeder krasse Boss braucht auch 'ne krasse rechte Hand, oder?~
IF ~~ THEN DO ~SetGlobal("NEEDUHELP","LOCALS",1)~ EXTERN ~NQ#SHUK~ 24
END

IF ~~ THEN BEGIN HELP4
SAY ~Du hast noch nich' mit Raki gequatscht, oder? Ich weiß genau welche Probleme der hat und jeder krasse Boss braucht auch 'ne krasse rechte Hand, oder?~
IF ~~ THEN DO ~SetGlobal("NEEDUHELP","LOCALS",1)~ EXTERN ~NQ#SHUK~ 24
END
...
SAY ~Ja, Boss? Hast'e Dir schon überlegt wie ich Dir helfen kann??~
IF ~~ THEN DO ~SetGlobal("NEEDUHELP","LOCALS",1)~ EXTERN ~NQ#SHUK~ 24
END
