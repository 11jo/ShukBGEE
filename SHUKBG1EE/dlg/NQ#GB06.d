...
IF ~~ THEN BEGIN 0
SAY @0 /* Der Eine hat Euch nicht vergessen, Shuk Worgreiter! Ich habe den anderen gesagt, dass Ihr kommt, aber überzeugen müsst Ihr sie selbst! */
IF ~~ THEN EXTERN ~NQ#SHUK~ 7
END

IF ~~ THEN BEGIN 1
SAY @1 /* Überzeugt die drei Hauptmänner an den anderen Lagerfeuern, dass Ihr der Held seid auf den wir gewartet haben! Ja, überzeugt sie! Dann werden wir Euch in das gelobte Land folgen, hahaha! */
IF ~~ THEN EXTERN ~NQ#SHUK~ 8
END

IF ~~ THEN BEGIN 2
SAY @2 /* Ja, hahaha! Das gelobte Land wartet auf uns! */
IF ~~ THEN EXTERN ~NQ#SHUK~ 23
END

IF ~Global("NQ#GOBLINCREW","GLOBAL",50)~ THEN BEGIN LOSE
SAY @3 /* Ihr habt es versaut, Abschaum. */
IF ~~ THEN EXIT
END

IF ~!Dead("NEROGDOK")~ THEN BEGIN IKNOW
SAY @4 /* Ihr seid zu früh dran, Abschaum. Vielleicht wird Vielauge mit Euch sprechen, wenn Ihr Rogdok getötet habt. */
IF ~~ THEN EXIT
END

IF ~Dead("NEROGDOK") Global("NQ#GOBLINCREW","GLOBAL",1) NumberOfTimesTalkedTo(0) !Name("NQ#SHUK",LastTalkedToBy)~ THEN BEGIN HESDEAD0
SAY @5 /* Abschaum, da seid Ihr! Sehr gut, dann ist dieser Teil Eurer Bestimmung erfüllt! Rogdok ist tot und der falsche Gott vertrieben! */
...
SAY @6 /* Da ist der Shuk! Ich wusste Ihr würdet kommen! Ich habe Euch gesehen... Krieger. */
IF ~~ THEN GOTO HISHUK
END

IF ~Dead("NEROGDOK") Global("NQ#GOBLINCREW","GLOBAL",1) !NumberOfTimesTalkedTo(0)~ THEN BEGIN HESDEAD1
SAY @7 /* Der Abschaum ist zurück, sehr gut, dann ist dieser Teil Eurer Bestimmung erfüllt! Rogdok ist tot und der falsche Gott vertrieben! */
IF ~See("NQ#SHUK") !StateCheck("NQ#SHUK",CD_STATE_NOTVALID)~ THEN GOTO HISHUK
IF ~!PartyHasItem("NQ#SHUK") OR(2) !See("NQ#SHUK") StateCheck("NQ#SHUK",CD_STATE_NOTVALID)~ THEN GOTO HESDEAD2
IF ~PartyHasItem("NQ#SHUK") OR(2) !See("NQ#SHUK") StateCheck("NQ#SHUK",CD_STATE_NOTVALID)~ THEN DO ~EraseJournalEntry(@70016) AddJournalEntry(@70017,QUEST)~ GOTO HESDEAD3
END
...
SAY @8 /* Vielauge hat Euch gesehen, <RACE>. Ihr werdet uns einen neuen Anführer bringen! Oder... waren es Murmeln? Bah! Hahahahaha! */
IF ~~ THEN DO ~AddJournalEntry(@70016,QUEST) SetGlobal("NQ#GOBLINCREW","GLOBAL",2)~ EXIT
END

IF ~~ THEN BEGIN HESDEAD3
SAY @9 /* Ah! Ihr habt das Artefakt! Hahaha! Bringt uns den Auserwählten! */
IF ~~ THEN REPLY @10 /* Hm... meint Ihr den Goblinstein? */ DO ~SetGlobal("NQ#GOBLINCREW","GLOBAL",3)~ GOTO HESDEAD4
IF ~~ THEN REPLY @11 /* Ja, klar. Ich bin gleich wieder da... */ DO ~SetGlobal("NQ#GOBLINCREW","GLOBAL",3)~ EXIT
END

IF ~~ THEN BEGIN HESDEAD4
SAY @12 /* Ja, jaaa, jahahahahaha! */
IF ~~ THEN EXIT
END

IF ~Global("NQ#GOBLINCREW","GLOBAL",2)~ THEN BEGIN NOSHUK1
SAY @13 /* Vielauge sieht... Vieles! Muhahaha! */
IF ~See("NQ#SHUK") !StateCheck("NQ#SHUK",CD_STATE_NOTVALID)~ THEN GOTO HISHUK
...
SAY @14 /* Wo ist der Shuk!? Hahaha! Wo!? */
IF ~~ THEN EXIT
END
...
SAY @15 /* Ihr seid ein wahrer Goblin, nicht wahr? Ein Anführer? Hahaha! */
...
SAY @16 /* Hihihihi, der Shuk hat sie alle überzeugt! Gut, gut! Die Zeit der Zweifler ist vorbei! Jetzt sehen alle, dass Feuerauge der einzige Gott ist den wir brauchen! */
...
SAY @17 /* Hihihihi, der Shuk hat sie alle überzeugt! Naja, oder wenigstens anständig umgebracht, hahaha! Gut, gut! Die Zeit der Zweifeller ist vorbei! Jetzt sehen alle, dass Feuerauge der einzige Gott ist den wir brauchen! */
IF ~~ THEN EXTERN ~NQ#SHUK~ 22
END

IF ~Global("NQ#GOBLINCREW","GLOBAL",4)~ THEN BEGIN QUEST0
SAY @18 /* Sucht die Hauptmänner! Die jämmerlichen Schlappschwänze hocken an den Feuern! */
IF ~~ THEN EXIT
END